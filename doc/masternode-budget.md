...
Needycoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<needycoin_address\> \<monthly_payment_needycoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ Needycoin, _12_ times over the course of a year totaling _7777_ Needycoin.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<needycoin_address\> \<monthly_payment_needycoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` Needycoin.
