...
#include "config/needycoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("needycoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "needycoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "needycoind.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, needycoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Needycoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:8888"));
    ...
    strUsage += HelpMessageOpt("-backupzneedycoin=<n>", strprintf(_("Enable automatic wallet backups triggered after each zNeedycoin minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeneedycoinamount=<n>     " + strprintf(_("Keep N ART anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Needycoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_(" ")) + "\n" +
           "\n" +
...
{
    RenameThread("needycoin-loadblk");
...
/** Sanity checks
 *  Ensure that Needycoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Needycoin Core is shutting down."));
...
#endif
    // Make sure only a single Needycoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Needycoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Needycoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Needycoin: load previous sessions sporks if we have them.
                ...
                // Needycoin: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Needycoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Needycoin Core to complete") << "\n";
        ...
        bool fEnableZNeedycoinBackups = GetBoolArg("-backupzneedycoin", true);
        pwalletMain->setZNeedycoinAutoBackups(fEnableZNeedycoinBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeNeedycoinAmount = GetArg("-anonymizeneedycoinamount", 0);
//    if (nAnonymizeNeedycoinAmount > 999999) nAnonymizeNeedycoinAmount = 999999;
//    if (nAnonymizeNeedycoinAmount < 2) nAnonymizeNeedycoinAmount = 2;
    ...
    LogPrintf("Anonymize Needycoin Amount %d\n", nAnonymizeNeedycoinAmount);
