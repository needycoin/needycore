...
#include "zneedycoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zned ought to be enough for anybody." - Bill Gates, 2017
    ui->znedpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelznedSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzned->setEnabled(false);
        ui->pushButtonMintzned->setToolTip(tr("zned is currently disabled due to maintenance."));

        ui->pushButtonSpendzned->setEnabled(false);
        ui->pushButtonSpendzned->setToolTip(tr("zned is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->znedpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzned_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zned is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zned...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zned in ") +
...
void PrivacyDialog::on_pushButtonSpendzned_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zned is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zned
        sendzned();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zned
    sendzned();
}

void PrivacyDialog::on_pushButtonZNeedycoinControl_clicked()
{
    ZNeedycoinControlDialog* zNeedycoinControl = new ZNeedycoinControlDialog(this);
    zNeedycoinControl->setModel(walletModel);
    zNeedycoinControl->exec();
}

void PrivacyDialog::setZNeedycoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNeedycoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzned()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Needycoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->znedpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zned is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zned</b>";
    ...
    // use mints from zNeedycoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNeedycoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNeedycoinControlDialog::GetSelectedMints();
    }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zned transaction
    ...
    // Clear zneedycoin selector in case it was used
    ZNeedycoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNeedycoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Needycoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNeedycoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zned </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zned "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zned "));
    ui->labelznedAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelznedSyncStatus->setVisible(fShow);
}
