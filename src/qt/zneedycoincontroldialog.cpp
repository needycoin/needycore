...
#include "ui_zneedycoincontroldialog.h"
...
std::list<std::string> ZNeedycoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNeedycoinControlDialog::listMints;

ZNeedycoinControlDialog::ZNeedycoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNeedycoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNeedycoinControlDialog::~ZNeedycoinControlDialog()
{
    delete ui;
}

void ZNeedycoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNeedycoinControlDialog::updateList()
{
...
void ZNeedycoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNeedycoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNeedycoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNeedycoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNeedycoinControlDialog::GetSelectedMints()
{
...
void ZNeedycoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
