...
// Copyright (c) 2017-2018 The Needycoin developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total needycoin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in needycoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in needycoin/kb\n"
    ...
    Object zneedycoinObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zneedycoinObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zneedycoinObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"needycoinaddress\"\n"
            "\nReturn information about the given needycoin address.\n"
            "\nArguments:\n"
            "1. \"needycoinaddress\"     (string, required) The needycoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"needycoinaddress\", (string) The needycoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Needycoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are needycoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) needycoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"needycoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"needycoinaddress\"  (string, required) The needycoin address to use for the signature.\n"
...
Value getstakingstatus(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain) {
        ...
        obj.push_back(Pair("enoughcoins", (pwalletMain->GetBalance() > 500 * 100 * COIN && nReserveBalance <= pwalletMain->GetBalance())));
    }
